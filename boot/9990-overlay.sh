...
setup_unionfs ()
{
	...
	if [ -n "${COWSIZE}" ]
	then
		mount -t tmpfs -o size="${COWSIZE}" tmpfs /live/overlay
	else
	...
	then
		### Force mdadm scan and format live-rw partition 
		check_dev "null" "/dev/md0" "skip_uuid_check"
		sleep 3
                if [ -e /dev/md0 ]
		then
			for partition in $(ls /dev/md0*) 
			do
				local root_label="cowroot"
				if [ "$(/sbin/blkid -s PART_ENTRY_NAME -p -o value ${partition} 2>/dev/null)" = "${root_label}" ]  && [ "${FORMATCOW}" = "Yes" ]
				then
					local mntpoint=/tmp/cow
                                	/bin/mkfs.ext4 -F -L persistence "${partition}"
					sleep 3
					mkdir -p "${mntpoint}" 
					mount -t ext4 "${partition}" "${mntpoint}"
					echo "/ union" > "${mntpoint}/persistence.conf" 
					umount "${partition}" 
                        	fi
			done
		fi
	...
	if [ -n "${COWSIZE}" ]
               	then
		cow_mountopt="rw,noatime,mode=755,size=${COWSIZE}"
	else
	...
	then
		...
		then
			mount -t tmpfs -o ${cow_mountopt} tmpfs "/live/overlay"
